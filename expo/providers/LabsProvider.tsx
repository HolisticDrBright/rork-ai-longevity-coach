...
// (removed: uploadPdfToOpenAI, callOpenAIWithFile, deleteOpenAIFile — lab PDFs now go through Supabase Edge Function)
void generateText;
void generateObject;
...
import { analyzeLabViaEdgeFunction } from '@/lib/labAnalyzerClient';
...
Be thorough and extract every biomarker visible in the document. Base ALL recommendations on the patient's actual biomarker values — not generic advice.`;
      ...
      if (isPdf) {
        console.log('[Labs] PDF detected — sending to lab-analyzer edge function (Textract + GPT enrichment)');
        try {
          const result = await analyzeLabViaEdgeFunction({
            fileUri,
            mimeType,
            fileName: fileName ?? 'lab-results.pdf',
            onProgress: (s) => console.log('[Labs] job status:', s),
          });
          ...
          console.log('[Labs] Edge function PDF analysis failed:', msg);
          throw new Error(
            msg.includes('signed in')
              ? 'Please sign in to analyze labs.'
              : msg.includes('5 MB') || msg.includes('too large')
              ? msg
