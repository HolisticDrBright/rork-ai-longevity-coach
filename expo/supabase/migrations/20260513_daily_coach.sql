...
--
-- The actual context tables (profiles, clinical_intakes, contraindications,
-- symptom_logs, lab_analysis_jobs, daily_biometric_records, meal_logs,
-- daily_nutrition_rollups, daily_recommendations) already exist. This
-- migration adds:
--   1. supplement_contraindication_rules  — declarative safety gates
--   2. coach_run_logs                      — full audit of each coach run
--   3. Indexes the daily-coach edge function needs for fast aggregation
--   4. Seed rows for the most common contraindications
...
-- ============================================================
-- 2. coach_run_logs (audit trail of every daily-coach invocation)
-- ============================================================
...
--    These are practitioner-editable later; values are starting points,
--    not clinical gospel. Severity 'block' = never recommend; 'caution' =
--    LLM can include with explicit warning.
-- ============================================================

-- Pregnancy blocks (hormone-affecting and uterotonic compounds)
...
-- Nursing blocks (subset that pass into breastmilk or affect supply)
...
-- Biomarker-driven blocks (the DHEA-when-DHEA-already-high case)
...
-- Symptom-pattern blocks (e.g. signs of high androgens → block androgen support)
  ('DHEA', 'symptom_pattern', '{"any_of":["acne","hirsutism","male_pattern_hair_loss","jaw_acne","oily_skin"]}', 'caution',
    'Symptoms consistent with elevated androgens; DHEA may worsen.'),
  ('Tribulus', 'symptom_pattern', '{"any_of":["acne","hirsutism","male_pattern_hair_loss","oily_skin"]}', 'caution',
...
-- Touch updated_at on rule edits
